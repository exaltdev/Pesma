...
ssize_t pesma_internal_write(PHandle* handle, size_t size, void* value, const char* message)
{
    ...
    if(handle->write_buffer.buffer_pos + size >= handle->write_buffer.buffer_size) {
    ...
    while(--size >= 0) {
        ...
        data[handle->write_buffer.buffer_pos++] = byte;
    }
    return 0;
}
...
uint64_t pesma_internal_read(PHandle* handle, size_t size, const char* message)
{
    ...
    while(--size >= 0) {
        uint8_t byte = data[handle->read_buffer.buffer_pos++];
        value |= byte << size * 8;
    }
    return value;
}
